...
void admin_manage_drugs()
{
    ...
    if (!prompt_yes_no("是否修改某药品库存或预警线？"))
    {
        printf("按任意键返回...");
        getchar();
        return;
...
void admin_manage_staff()
{
    while (1)
    {
        ...
        if (choice == 3)
        {
            ...
            {
                char msg[100];
                snprintf(msg, sizeof(msg), "确认删除员工 %s（工号%d）？", target->name, target->id);
...
void admin_manage_patients()
{
    while (1)
    {
        ...
        if (choice == 1)
        {
            ...
            {
                printf("%-8d %-10s %-18s %s\n",
                       cur->medical_id, cur->name, cur->id_card,
        ...
        else if (choice == 2)
        {
            ...
            while (cur != NULL)
            {
                if (cur->medical_id == medical_id)
                {
                    ...
                    if (cur->is_deleted)
                    {
                        printf("患者 %d 已经删除，无需重复操作。\n", medical_id);
                    }
                    else
                    {
                        printf("确认删除患者 %s（病历号：%d）？[y/n]: ", cur->name, medical_id);
                        char confirm;
                        scanf("%c", &confirm);
                        clear_input();
